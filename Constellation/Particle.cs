...
namespace Constellation
{
    public class ParticleEmitter
    {
        ...
        public ParticleEmitter(PointF loc, Color colorA, Color colorB, int population)
        {
            if (population > 0)
            {
                Random r = new Random();
				...
				for (int i = 0; i < n; i++) {
					...
					particles.Add(new Particle(loc + new Size(r.Next(-scatter, scatter), r.Next(-scatter, scatter)),
						r.Next(0, 360), c, (float)Math.Max(1, (float)Math.Sqrt(population) / 10) * 1f, r.Next(2, 6)));
    ...
    public class Particle
    {
        ...
        PointF movement; public bool shouldRemove = false;
